# EDR-E001@1 — Sealed EPOCH-001 Development Report

**Epoch:** `EPOCH-001 — Post-closure Jester finalization`  
**State:** SEALED / FINALIZED PUBLICATION  
**Sealed:** 2026-09-15  
**Accepted closure audit:** `_mw/epochs/001-jester-finalization/results/CLOSURE-AUDIT-E001-01.md` — `PASS`  
...
**Audited candidate payload blob:** `99dd0c58e730ea028fd6c73c2654a479aeb17d90`  
**Canonical published locator:** `development-reports/epoch-001-jester-finalization.md`

This publication seals the exact MADARAII-35 candidate payload audited by `CLOSURE-E001-01`. The payload below is preserved **verbatim**, including its pre-seal lifecycle line saying `closure-audit candidate; unsealed`; that line describes the candidate at the audit cutoff. This sealing wrapper supersedes only that lifecycle status. No substantive report claim is rewritten during finalization.

The current Product, Target, Scientific Knowledge, Product Architecture, Decision, Question and Evidence owners retain semantic precedence for current engineering truth. This report is the durable historical synthesis of the epoch, not a second editable Product/Knowledge owner.
...
**Cutoff Product Architecture:** `PA-0001 revision 3`  
...
This Report owns the epoch-level historical synthesis only. Current Product, Knowledge, Decision, Architecture, Evidence, Question and Work-State meaning remain in their semantic owners.
...
At epoch start Product `b2caf...` was current, Target WHAT/HOW were revision 2, Scientific Knowledge was established, and Product Architecture `PA-0001 revision 2` allocated Shell/Editor/DOCX/PDF/Storage/Build-Install/Verification responsibilities. Q-0006 interactive Windows 11 qualification and Q-0007 broad real-document compatibility were explicit reliance limits, not blockers. Release, deployment and user-workstation installation were outside the epoch state.
...
The user-facing MiniDoc 0.1 feature boundary did not expand. The epoch strengthened correctness, authority boundaries and evidence around existing behavior:
...
- dual direct-save plus recovery failure reports target-integrity uncertainty explicitly;
- asynchronous Open requests gained monotonic user-intent freshness;
- replacement authority is also bound to the document-content generation captured after unsaved-change handling;
- stale asynchronously acquired PDF candidates are disposed rather than admitted;
- verification-mode success requires successful selected-document admission and successful initial PDF render, while failure is noninteractive and non-zero.
...
Target HOW evolved from revision 2 to revision 5. Early reconciliation removed overstated compatibility-preview/search mechanisms; the first repair incorporated candidate-first replacement, PDF freshness, table geometry and save-recovery semantics; revision 5 added Open intent/content-generation authority fencing and the strengthened installed PDF success oracle.
...
The session attacked state/authority assumptions beyond ordinary verification. It retained six source/design observations: premature replacement-session teardown; stale PDF render completion; table descriptor divergence; compatibility preview/notes overstatement; nonexistent case-sensitive UI toggle despite HOW wording; and hidden target-integrity uncertainty when both save and restoration fail.

Observation mode performed no Product/Knowledge/external mutation, so no restoration was required. Mandatory continuation developed the Report under MADARAII-03 and reconciled every act under MADARAII-04 before specialized follow-up. Overstated mechanisms were corrected rather than converted into features; justified Product repairs were planned, readiness-reviewed, implemented, independently verified and admitted. Exact candidate `4a7c1d5...` became current through `INTEGRATION-E001-01`.
...
Prior Jester creative framing was excluded. The new pass found a distinct authority class: render callbacks were fenced, but asynchronous candidate **Open** admission was not. A slow earlier PDF Open could finish after a later New/Open intent and become current by completion order.

It also found that Save/Discard permission could become stale while PDF loading was pending: the still-active DOCX could be edited again, and late admission could otherwise discard the newer edit without a fresh authority check.

Additional signals were a Target WHAT vs HOW/Product compatibility mismatch and an installed lifecycle oracle gap: process exit after attempted PDF path did not itself prove successful installed UI admission/render. Memory cost of overwriting an enormous existing target was terminally retained as a bounded fragility/no-action item; a table insertion-vs-growth asymmetry established no target contradiction and was also no-action.

Observation mode again mutated no authoritative state. `_mw/epochs/001-jester-finalization/intake/JR-E001-02-developed.md` plus MADARAII-04 reconciliation completed mandatory continuation before any repair was authorized.

## Second repair mechanism and producer deviation

`WHAT-E001-02` / `HOW-E001-02` reconciled target semantics first. `PLAN-E001-02` then bounded implementation to asynchronous Open authority and installed verification; `READINESS-E001-02` found it ready.

The implementation introduced `OpenTransitionGuard` with two monotonic dimensions: an **intent generation** for which Open/New replacement intent is current, and a **content generation** for whether the active editable document changed after an Open ticket captured replacement authority. The Shell rechecks both at admission. DOCX admission occurs before current-session release; stale PDF candidates are rejected/disposed; existing PDF render freshness remains a separate post-admission fence.

The verification-only startup path now returns actual success/failure. PDF success requires successful first render. `--verify-close-after-open` suppresses modal failure blocking and exits non-zero on failure. Windows lifecycle verification requires `ExitCode == 0`.

The first producer candidate `2578a32599aa451b31d3db213c3e4f481911d655` failed at build/semantic checks because the new check referenced internal `OpenTransitionGuard` without importing namespace `MiniDoc`. This compile-only check-project defect was corrected inside Plan scope; final candidate `1df3b565...` retained the same design. The failed candidate was never verified or admitted.

## Verification, evidence and final admission

`VERIFICATION-E001-02` independently reconstructed the second repair claims from Plan/Target and exact candidate source and concluded `CONFORMS` within the bounded repair scope. Deterministic guard checks establish later-intent supersession and content-generation invalidation; exact shell inspection establishes those checks at the real admission boundary and stale PDF candidate disposal; render-request/session fencing protects stale render completion; verification-mode source establishes success only after initial render and noninteractive non-zero failure.
...
`INTEGRATION-E001-02` established identity-preserving admission: later commits after candidate freeze affected only Work/Evidence/owner records, so exact verified `1df3b565...` became current Product without post-verification Product transformation.

## Evidence limits and protected non-claims

No probabilistic/live WPF timing-stress harness was introduced. Open race protection is supported by deterministic state-machine checks and exact source/control-flow reconstruction, while installed success behavior is executed separately on Windows CI. A future editable mutation mechanism bypassing content-generation fencing reopens the affected claim.
...
These and this Report are historical/navigation products, not current Product/Knowledge owners.
...
Version-control history remains the deep route for exact source/configuration revisions and failed producer attempt lineage.
